...
//YYYY/MM
const currentYYYYMMButton = document.getElementById('current-date-yyyy-mm') as HTMLButtonElement;
currentYYYYMMButton.addEventListener('click', () => {
  const now = new Date();
  const year = now.getFullYear();
  const month = String(now.getMonth() + 1).padStart(2, '0');
...
//YY/MM/DD
const currentYYMMDDButton = document.getElementById('current-date-yy-mm-dd') as HTMLButtonElement;
currentYYMMDDButton.addEventListener('click', () => {
    const now = new Date();
    const year = String(now.getFullYear()).slice(-2);
    const month = String(now.getMonth() + 1).padStart(2, '0');
    const day = String(now.getDate()).padStart(2, '0');
